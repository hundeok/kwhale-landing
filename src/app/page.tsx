      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-0 flex items-center justify-between h-16">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-cyan-200 text-xs leading-none mt-1">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-cyan-50/60 text-sm hover:text-white transition-colors"
            >
              {item.name}
            </a>
          ))}
...
function KWHero({
  ...
  return (
    <motion.section
      id="overview"
      className="min-h-screen flex items-center justify-center pt-20 pb-28 relative"
      style={{ scale, opacity }}
    >
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            {CATEGORY_PILLS.map((c) => (
              <div
                key={c.value}
                className={`rounded-xl px-3 py-2 bg-slate-950/30 border border-cyan-400/5 flex flex-col gap-1 ${
                  c.negative ? 'ring-1 ring-red-500/20' : ''
                }`}
              >
                <span className="text-xs text-slate-200/65">{c.label}</span>
                <span className={`text-sm font-semibold ${c.negative ? 'text-red-200' : 'text-cyan-100'}`}>
                  {c.amount}
                </span>
              </div>
            ))}
          </div>
          <div className="mt-6 text-xs text-slate-200/50">
            * 채무는 기본적으로 합산에서 제외되도록 세팅되어 있어요.
          </div>
        </motion.div>
      </div>
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-slate-200/70 mb-4">
              기관별·연도별로 포맷이 달라도 일단 그대로 받아둡니다.
            </p>
            <pre className="text-[0.6rem] bg-slate-950/40 p-3 rounded-lg text-slate-100/70">
              {`{ id, personId, assetType, amount, location, declaredAt }`}
            </pre>
          </motion.div>
          {/* 2) Core Entity */}
          <motion.div
          ...
          >
            ...
            <p className="text-sm text-slate-200/70 mb-4">
              “예금/증권/코인/부동산/차량/채무” 이렇게 우리가 쓰는 축으로만 다시 묶습니다.
            </p>
            <pre className="text-[0.6rem] bg-slate-950/40 p-3 rounded-lg text-slate-100/70">
              {`KWAsset { category, subCategory, amount, declaredPeriod, locationId }`}
            </pre>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-slate-200/70 mb-4">
              “이 지역이 얼마인지”, “이 종목 누가 갖고 있는지” 같은 질문을 바로 답할 수 있게 만듭니다.
            </p>
          ...
          >
            ...
            <p className="text-sm text-slate-200/70 mb-4">
              임원/의사결정자가 바로 볼 수 있는 KPI 뷰로 바꿉니다.
            </p>
            <pre className="text-[0.6rem] bg-slate-950/40 p-3 rounded-lg text-slate-100/70">
          ...
          {SCREEN_CARDS.map((sc) => (
            ...
            >
              ...
              <p className="text-slate-200/40 text-xs">
                데이터가 한 번만 정리되면 이 화면들은 그냥 선택지일 뿐입니다.
              </p>
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-0">
        ...
        >
          ...
          <p className="text-slate-200/70 mb-8">
            ...
            내부 PoC·사내 대시보드·외부 서비스 어디에 붙이든 같은 톤으로 유지됩니다.
          </p>
          <div className="flex flex-wrap gap-4">
            <button className="bg-cyan-400 text-slate-950 font-semibold px-5 py-2.5 rounded-lg hover:brightness-110 transition">
              샘플 데이터 요청
            </button>
            <button className="border border-cyan-200/40 text-cyan-50 px-5 py-2.5 rounded-lg hover:bg-cyan-50/5 transition">
              도입 상담 일정 잡기
            </button>
          </div>
        </motion.div>
